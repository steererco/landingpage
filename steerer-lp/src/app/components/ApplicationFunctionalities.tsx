...
export const ApplicationFunctionalities = ({
  ...
  return (
    <div
      ref={myRef}
      className={`snap-start bg-white mobile:h-screen flex flex-col items-center justify-center mt-[111px] mobile:mt-0 mobile:mb-20 px-5 py-16 mobile:px-20 mobile:py-0 transition-opacity ease-in duration-700 ${
...
const SmallTextBlock = ({ children }: { children: ReactNode }): JSX.Element => (
  <div className="flex bg-gray1/[.1] h-1/3 mt-4 items-center py-[30px] mobile:py-0 px-8 rounded-[20px]">
    <p className="text-black font-poppins font-medium text-base mobile:text-2xl">
      {children}
    </p>
  </div>
);
...
const BigTextBlock: React.FC<BigTextBlockProps> = ({
  title,
  description,
}): JSX.Element => (
  <div className="bg-gray1/[.1] h-2/5 py-[30px] mobile:py-0 flex flex-col justify-center px-8 rounded-[20px]">
    ...
    <p className="text-black/[.7] font-inter mt-1 text-xs mobile:text-base">
      {description}
    </p>
  </div>
);
...
const ImageBlock = (): JSX.Element => (
  <div className="relative flex items-end justify-center bg-gray1/[.1] px-[63px] mobile:px-0 pt-[25px] mobile:pt-0 mobile:w-2/5 mobile:mr-4 rounded-[20px]">
    ...
    <Image
      src={mockup}
      alt="Functionalities"
      className="bg-white px-5 pt-[19px] h-[92%]"
    />
  </div>
);
...
const PlayerCard = (): JSX.Element => (
  <div className="absolute flex flex-row items-center justify-start w-[110px] h-[39px] mobile:w-[190px] mobile:h-[66px] bg-base-blue2 rounded-[6px] mobile:rounded-[10px] top-[76px] right-[17px] mobile:top-[220px] mobile:right-[29px]">
    ...
    <div className="flex flex-col items-start ml-2 mobile:ml-3 font-poppins">
      ...
      <p className="text-[7px] mobile:text-sm font-medium text-white/[.7]">
        24 Goals
      </p>
    </div>
  </div>
...
const TextArea = (): JSX.Element => (
  <>
    <p className="text-black text-center font-poppins font-medium text-[32px] max-w-[1061px] mobile:text-5xl">
      Seize the opportunity to advance your skills and knowledge, easily
      accessible.
    </p>
    <p className="text-black/[.7] max-w-[874px] text-center mt-8 font-inter font-normal text-base mobile:text-xl">
      With our app, you'll access professional soccer team strategies, precise
      analyses, and a community of players and coaches who share your drive to
      win.
    </p>
  </>
);
