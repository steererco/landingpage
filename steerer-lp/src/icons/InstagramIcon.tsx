import React from "react";

export const InstagramIcon = ({ ...props }: React.SVGProps<SVGSVGElement>): JSX.Element => (
    <svg
        width="22"
        height="22"
        viewBox="0 0 22 22"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        {...props}
    >
        <path d="M16 1H6C3.23858 1 1 3.23858 1 6V16C1 18.7614 3.23858 21 6 21H16C18.7614 21 21 18.7614 21 16V6C21 3.23858 18.7614 1 16 1Z" stroke="black" stroke-linecap="round" stroke-linejoin="round" />
        <path d="M15 10.3701C15.1234 11.2023 14.9813 12.0523 14.5938 12.7991C14.2063 13.5459 13.5932 14.1515 12.8416 14.5297C12.0901 14.908 11.2385 15.0397 10.4078 14.906C9.57714 14.7723 8.80977 14.3801 8.21485 13.7852C7.61993 13.1903 7.22774 12.4229 7.09408 11.5923C6.96042 10.7616 7.09208 9.90995 7.47034 9.15843C7.8486 8.40691 8.4542 7.7938 9.201 7.4063C9.9478 7.0188 10.7978 6.87665 11.63 7.00006C12.4789 7.12594 13.2649 7.52152 13.8717 8.12836C14.4785 8.73521 14.8741 9.52113 15 10.3701Z" stroke="black" stroke-linecap="round" stroke-linejoin="round" />
        <path d="M16.5 5.5H16.51" stroke="black" stroke-linecap="round" stroke-linejoin="round" />
    </svg>
)